...
import MyNFTTokenList from "./MyNFTTokenList";

export default function MyNFT({ web3, caver }) {
  ...
  useEffect(() => {
    ...
    async function saveMyToken() {
      ...
      const MyTokens = await tokenContract.methods.getNftTokens(account).call();
      const items = await Promise.all(MyTokens.map(async i => {
